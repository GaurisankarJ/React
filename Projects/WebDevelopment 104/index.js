...
class App extends React.Component {
    ...
    */
    ...
    }).attr("width", 25).attr("height", d => d * 3).attr("class", "bar").append("title").text(d => (d + " units"));
    //Add and style labels for elements
    svg.selectAll("text").data(dataset).enter().append("text").attr("x", (d, i) => {
        return i * 30;
    }).attr("y", (d, i) => {
        return 120 - d * 3 - 3;
    }).text(d => d).style("font-size", "20px").attr("fill", "grey")
    //Display shapes with SVG, change the color of an SVG element, add tooltip
    svg.append("rect").attr("width", 100).attr("height", 50).attr("x", 300).attr("y", 25).attr("fill", "navy").append("title").text("Rectangle");
    ...
    scatterSVG.selectAll("text").data(circleDataset).enter().append("text").attr("x", d => (padding + 5 + d[0])).attr("y", d => (500 - d[1] - padding - 5)).text(d => (d[0] + ", " + d[1]));
    ...
    var max = d3.max(positionData, d => d[2]);//For set [-4, 8, 3]
    var min = d3.min(positionData, d => d[1]);//For set [7, 3, 8]
    //Use dynamic scales
    const w = 500;
    const h = 500;
